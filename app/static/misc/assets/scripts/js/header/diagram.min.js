...
(function() {
    ...
    Diagram.prototype.resetDiagram = function() {
        ...
        _.options.box.find('.svg-wrap, div[class*="comp-diagram__value"], div[class*="large-diagram__value"]').remove();
    };
    ...
    Diagram.prototype.tripleDiagram = function() {
		...
		var _ = this, radius, diff, degrees, val, path = [], radiusArr = [], valArr = [], coords = [];
        ...
        for (var i = 0; i < 4; i++) {
            ...
            switch (i) {
                ...
                case 0:
				diff = _.options.data[0] - _.options.data[1];
				if (diff <= 0) {
					radius = (_.options.viewBox / 2) - (_.options.stroke / 2);
					degrees =  (_.options.data[0] + Math.abs(diff)) / _.degree;
				} else {
					radius = (_.options.viewBox / 2) - (_.options.stroke / 2) - _.options.stroke;
					degrees =  (_.options.data[1] + Math.abs(diff)) / _.degree;
				}
				val = diff;
                break;
			...
			coords[i] = [_.x0, _.y0, _.x1, _.y1];
            radiusArr[i] = radius;
            valArr[i] = val.toFixed(1).replace('.', ',').replace(/(\d)(?=(\d\d\d)+([^\d]|$))/g, '$1 ');
        }

        return {path: path, radius: radiusArr, val: valArr, preV: _.options.data[1].toFixed(0), coords: coords };
    };
    ...
    Diagram.prototype.ifiniteDiagram = function() {
        ...
        var _ = this, radius, degrees, val, path = {Dots:[], Gradient:[], percValue:null, faktVal:null, zero:false};
        ...
        val = _.options.data[0];
        path.faktVal = val.toFixed(1).replace('.', ',').replace(/(\d)(?=(\d\d\d)+([^\d]|$))/g, '$1 ');
    ...
    Diagram.prototype.segmentDiagram = function() {
        ...
        for (var i = 0; i < _.options.data.length; i++) {
			...
			if (degrees >= 360 || (degrees == 0 && _.options.data.length == 1)) fulldegrees = true;
			if (degrees >= 300) highdegrees = true;
			
			if (fulldegrees) degrees = 359.999;

            path.Dots[i] = _.calculatePath(degrees, radius-_.options.padding);
			
			if (highdegrees) {
				_.x1 = Math.cos( (_.startAngle + 300) * _.rad ) * (radius-_.options.padding) + (_.options.viewBox / 2);
				_.y1 = Math.sin( (_.startAngle + 300) * _.rad ) * (radius-_.options.padding) + (_.options.viewBox / 2);
			}
    ...
    Diagram.prototype.dashedDiagram = function() {
        ...
        var _ = this, radius, degrees, overVal, path = {Dots:[], Gradient:[]};
		...
		if (_.options.data[1] >= _.options.data[0]) {
			overVal = true;
		}
        ...
        for (var i = 0; i < 2; i++) {
            ...
            if (i == 0) {
				...
				degrees =  (_.options.data[0] - 1) / _.degree;
			} else {
                degrees =  _.options.data[i] / _.degree;
            }
    ...
    Diagram.prototype.createSvg = function(path){
        ...
        if( _.options.kind == 'triple' ){
			...
			var defProfPathVal = parseInt(path.val[0]), colors, defProfGradX, defProfGradY, defProfClass;
			
			if (defProfPathVal < 0) {
				colors = ['#30B069', '#FEE300'];
				defProfGradX = [path.coords[2][2], path.coords[0][2]];
				defProfGradY = [path.coords[2][3], path.coords[0][3]];
				defProfClass = 'comp-diagram__num_yellow';
			} else {
				colors = ['#FEE300', '#30B069'];
				defProfGradX = [path.coords[1][2], path.coords[0][2]];
				defProfGradY = [path.coords[1][3], path.coords[0][3]];
				defProfClass = 'comp-diagram__num_yellow comp-diagram__num_pos';
			}

            values = '<div class="comp-diagram__values"><div id="value-path-'+ _.element +'-2" class="comp-diagram__num comp-diagram__num_blue diagram-value" data-value="'+ path.val[2] +'">'+ path.val[2] +'</div><div id="value-path-'+ _.element +'-1" class="comp-diagram__num comp-diagram__num_pink diagram-value" data-value="'+ path.val[1] +'">'+ path.val[1] +'</div><div id="value-path-'+ _.element +'-0" class="comp-diagram__num '+ defProfClass +' diagram-value" data-pre-val="'+ path.preV +'" data-value="'+ path.val[0] +'">'+ path.val[0] +'</div></div>';
            ...
            svgBody += '<filter id="f1"><feGaussianBlur stdDeviation="5"/></filter><defs><linearGradient xmlns="http://www.w3.org/2000/svg" id="svg-gradient_'+ _.element +'-0" gradientUnits="userSpaceOnUse" x1="'+ defProfGradX[0] +'" y1="'+ defProfGradY[0] +'" x2="'+ defProfGradX[1] +'" y2="'+ defProfGradY[1] +'"><stop offset="0" style="stop-color:'+colors[0]+'"/><stop offset="1" style="stop-color:'+colors[1]+'"/></defs><defs></linearGradient><linearGradient xmlns="http://www.w3.org/2000/svg" id="svg-gradient_'+ _.element +'-1" gradientUnits="userSpaceOnUse" x1="224.0493" y1="185.9902" x2="54.2576" y2="87.9609"><stop offset="0.1748" style="stop-color:#9D308A"/><stop offset="1" style="stop-color:#FF695B"/></linearGradient></defs><defs><linearGradient id="svg-gradient_'+ _.element +'-2" gradientUnits="userSpaceOnUse" x1="51.5933" y1="241.0088" x2="241.0492" y2="51.5528"><stop offset="0" style="stop-color:#00FFFF"></stop><stop offset="1" style="stop-color:#004CCF"></stop></linearGradient></defs>';
        ...
        if( _.options.kind == 'triple2' ){
            ...
            values = '<div class="comp-diagram__values"><div id="value-path-'+ _.element +'-2" class="comp-diagram__num comp-diagram__num_blue diagram-value" data-value="'+ path.val[2] +'+100">'+ path.val[2]  +' +100.0' +'</div><div id="value-path-'+ _.element +'-1" class="comp-diagram__num comp-diagram__num_pink diagram-value" data-value="'+ path.val[1] +'">'+ path.val[1] +' +100.0'  +'</div><div id="value-path-'+ _.element +'-0" class="comp-diagram__num '+ defProfClass +' diagram-value" data-pre-val="'+ path.preV +'" data-value="'+ path.val[0] +'">'+ path.val[0] +' +100.0'  +'</div></div>';
    ...
    if( _.options.kind == 'ifinite' ){
        ...
        values = '<div class="large-diagram__values"><div class="large-diagram__percent"><span id="value-perc-path-'+ _.element +'-0" class="diagram-value" data-value="'+ path.percVal +'">'+ path.percVal +'</span>%</div><div class="large-diagram__value large-diagram__value_top">план<p class="'+ _.options.valClass[0] +'">'+ _.options.maxValue.toFixed(1).replace('.', ',').replace(/(\d)(?=(\d\d\d)+([^\d]|$))/g, '$1 ') +'</p></div><div class="large-diagram__value large-diagram__value_bottom"><p id="value-path-'+ _.element +'-0" class="'+ _.options.valClass[1] +' diagram-value" data-value="'+ path.faktVal +'">'+ path.faktVal +'</p>факт</div></div>';
        ...
        svgBody += '<g filter="url(#filter-'+ _.element +'-0)"><path fill="none" stroke="#000" stroke-opacity="'+strokeOpacity[0]+'" stroke-width="'+ (_.options.stroke - 2) +'" stroke-linecap="round" d="'+ path.Dots[1] +' " class="anim-diagram anim-diagram_second" data-radius="'+ path.radius +'"/><rect x="0" y="0" width="100%" height="100%" fill="none" /></g>';

        svgBody += '<g><path id="path-'+ _.element +'-0" fill="none" stroke="url(#svg-gradient_'+ _.element +'-0)" stroke-opacity="'+strokeOpacity[1]+'" stroke-width="'+ _.options.stroke +'" stroke-linecap="round" d="'+ path.Dots[0] +' " class="anim-diagram anim-diagram_second anim-diagram_id" data-radius="'+ path.radius +'" /><rect x="0" y="0" width="100%" height="100%" fill="none" /></g>';
        ...
        $anim_obj.each(function() {
            ...
            for (var i = 0; i < val.length; i++) {
                ...
                if(val[i].length > 0){
                     ...
                     valueItem[i] = val[i];
                     dataValue[i] = valueItem[i].attr('data-value').replace(' ', '');
            ...
            setTimeout(function anim() {
                ...
                if( doffset > 0 ){
                    ...
                    for (var i = 0; i < v.length; i++) {
                        ...
                        if (v[i]) {
                            if (!last) {
                                ...
                                valueItem[i].html(curValue[i].toFixed(1).replace('.',',').replace(/(\d)(?=(\d\d\d)+([^\d]|$))/g, '$1 '));
                            } else {

                                if ( curValue[i] > stP[i] ) {
                                    valueItem[i].html(curPreValue[i].toFixed(1).replace('.',',').replace(/(\d)(?=(\d\d\d)+([^\d]|$))/g, '$1 '));
                    ...
                    for (var i = 0; i < v.length; i++) {
                        if (v[i]) {
                            valueItem[i].html(dataValue[i].replace(/(\d)(?=(\d\d\d)+([^\d]|$))/g, '$1 '));
                        }
                    }
                }
            ...
            if (doffset > 0) {
                ...
                for (var i = 0; i < v.length; i++) {
                    if (v[i]) {
                        ...
                        valueItem[i].html(dataValue[i].toFixed(1).replace('.',',').replace(/(\d)(?=(\d\d\d)+([^\d]|$))/g, '$1 '));
                    }
                }
...
//
	...
	$(document).on('mousemove', '.diagram-tabs__btn', function(){
		var id = $(this).attr('id').split('_')[1];
        $('#'+id).addClass('segment-diagram__sector_hover');
	});
	
	$(document).on('mouseleave', '.diagram-tabs__btn', function(){
		var id = $(this).attr('id').split('_')[1];
        $('.segment-diagram__sector').removeClass('segment-diagram__sector_hover');
	});
