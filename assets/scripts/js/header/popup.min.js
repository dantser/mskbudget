$(document).ready(function() {
    $(".open-popup").click(function() {
        ...
        if ($(e.target).parent().attr('id') == 'error') {
          return $("#error").fadeOut(321), !1
        } else {
          return $(".popup-window, #popup-wrapper").fadeOut(321), !1
        }
    });
    // $('.login-form__submit').each(function() {
      ...
      $('.login-form__submit').on('click', function(e) {
          ...
          // ФИО полностью, кириллица, с заглавной, допускается тире
          var fio = /([А-ЯЁ][а-яё]+[\-\s]?){3,}/.test($('#i-fio-1').val());
          var email = /^[-\w.]+@([A-z0-9][-A-z0-9]+\.)+[A-z]{2,4}$/.test($('#i-email-3').val());
          // Строчные и прописные латинские буквы, цифры, спецсимволы. Минимум 8 символов
          var password = /(?=^.{8,}$)((?=.*\d)|(?=.*\W+))(?![.\n])(?=.*[A-Z])(?=.*[a-z]).*$/.test($('#i-passw-1').val());

          if ($this.val() == 'Войти') {
            return
          } else if ($this.val() == 'Зарегистрироваться') {
              if (fio && email && password) {
                ...
                e.preventDefault();
                top.text('Ошибка');
                bottom.text('Введите корректные данные!');
                $("#popup-wrapper").fadeIn(321), $("#error").fadeIn(321),!1
                return
              }
