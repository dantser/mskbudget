		...
		switch ($mode)
		{
			case 'overview':
				...
				if ($submit)
				{
					...
					// Use gmmktime() function to create UNIX timestamp
					$time_offset = (isset($user->timezone) && isset($user->dst)) ? (int) $user->timezone + (int) $user->dst : 0;
					$update_time = gmmktime($hour, $minute, $second, $month, $day, $year) - $time_offset;
					...
					$data = array(
						...
						'user_regdate' => $update_time == null ? $user_row['user_regdate'] : $update_time,
					);
					...
					$update_regdate = ($data['user_regdate'] != $user_row['user_regdate']) ? $data['user_regdate'] : false;
					...
					if (!sizeof($error))
					{
						...
						*/
						...
						if ($update_regdate !== false){
							$sql_ary += array(
								'user_regdate'		=> $update_regdate,
							);
						}
						if ($update_password)
						{
